    ...
    public void init(){
        ...
        button.addActionListener(e -> {
            if(comboBox.getSelectedIndex() == 0){
                ...
                MainFrame.cdTable.fillTable(store.containsTitle(title));
            } else if(comboBox.getSelectedIndex() == 1){
                String collection = textField.getText();
                MainFrame.cdTable.fillTable(store.searchCollection(collection));
            } else if(comboBox.getSelectedIndex() == 2){
                String type = textField.getText().toUpperCase();
                MainFrame.cdTable.fillTable(store.searchType(type));
            } else if(comboBox.getSelectedIndex() == 3){
                try {
                    int price = Integer.parseInt(textField.getText());
                    MainFrame.cdTable.fillTable(store.searchPrice(price));
